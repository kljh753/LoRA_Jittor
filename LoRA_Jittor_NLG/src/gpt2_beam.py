...
from exp_utils import create_exp_dir, set_random_seed, AverageMeter, print_args
...
def _reorder_cache(past: Tuple, beam_idx) -> Tuple:
    return tuple(layer_past.gather(1, beam_idx.unsqueeze(-1).unsqueeze(-1).expand_as(layer_past)) for layer_past in past)
...
def beam(model, data_iter, args):
    ...
    with jt.no_grad():
        for idx, data in enumerate(data_iter):
            ...
            beam_scores = jt.zeros((batch_size, num_beams), dtype=jt.float32)

            best_sequence = jt.zeros((batch_size, args.eval_len), dtype=jt.int64)
            best_score = {}
            ...
            if args.rank == 0:
                _id = _id.view(-1).numpy()
                output = output.view(-1, output.shape[-1]).numpy()
                #score = score.view(-1, score.shape[-1]).numpy()
...
if __name__ == '__main__':
    ...
    # Note: Jittor doesn't have DistributedSampler, use regular DataLoader
    from jittor.dataset import DataLoader
