...
class Linear(nn.Linear, LoRALayer):
    ...
    def train(self, mode: bool = True):
        ...
        if mode:
            if self.merge_weights and self.merged:
                # Make sure that the weights are not merged
                if self.r > 0 and self.enable_lora.sum() > 0:
...
class MergedLinear(nn.Linear, LoRALayer):
    ...
    def __init__(
        ...
        self.fan_in_fan_out = fan_in_fan_out
        # Actual trainable parameters
        if r > 0 and self.enable_lora.sum() > 0:
            enable_count = int(self.enable_lora.sum())
            self.lora_A=self.weight.new_zeros((r * enable_count, in_features))
            # print(self.lora_A.shape, 'lllllla' * 50,enable_lora.sum().item())

            self.lora_B=self.weight.new_zeros((out_features // enable_lora.shape[0] * enable_count, r)
            ...
            self.lora_ind[jt.where(enable_lora)[0] , :] = True # Index assignment
        ...
        if fan_in_fan_out:
            # print('YES')
            self.weight = self.weight.transpose(0,1) # Key operation
            # print(self.weight.shape, 'aaaa' * 100, fan_in_fan_out)
    ...
    def zero_pad(self, x):
        ...
        # Use jt.where to get the indices where lora_ind is True
        true_indices = jt.where(self.lora_ind)[0]
        result[true_indices] = x
        return result
    ...
    def merge_AB(self):
        ...
        # Fix: Use .item() to get scalar value from Jittor Var
        try:
            # Try to get the scalar value properly
            enable_sum = self.enable_lora.sum()
            if hasattr(enable_sum, 'item'):
                groups = int(enable_sum.item())
            else:
                groups = int(enable_sum)
        except:
            # Fallback: calculate manually
            groups = int((self.enable_lora > 0).sum().item() if hasattr((self.enable_lora > 0).sum(), 'item') else (self.enable_lora > 0).sum())
        
        # Convert shape elements to integers to avoid NanoVector type error
        lora_A_expanded = self.lora_A.unsqueeze(0)
        lora_B_expanded = self.lora_B.unsqueeze(-1)
        
        in_channels = int(lora_A_expanded.shape[1])
        out_channels = int(lora_B_expanded.shape[0])
        kernel_size = int(lora_B_expanded.shape[2])
        
        # Memory optimization: avoid creating new Conv1d if groups is 0
        if groups == 0:
            return T(jt.zeros_like(self.weight))
        ...
        con1d=nn.Conv1d(
            ...
            groups=groups,
            bias=False # Fixed after debugging
        )
