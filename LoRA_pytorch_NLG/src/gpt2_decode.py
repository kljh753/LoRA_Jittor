...
if __name__ == "__main__":
    ...
    if args.ref_unique_file is not None:
        ...
        ref_unique = []
        uniques = {}
        with open(args.ref_unique_file, 'r', encoding='utf-8') as ref_unique_reader:
    ...
    with open(args.sample_file, 'r', encoding='utf-8') as sample_reader, \
             open(args.input_file, 'r', encoding='utf-8') as input_reader, \
             open(args.output_pred_file, 'w', encoding='utf-8') as pred_writer:
        ...
        for line in input_reader:
            ...
            if ref_unique is None:
                _key = context
                # Ensure _key is string type
                if isinstance(_key, list):
                    _key = ' '.join(str(item) for item in _key)
                elif not isinstance(_key, str):
                    _key = str(_key)
            else:
        ...
        for line in sample_reader:
            ...
            if _key in refer_dict:
                refer_dict[_key]['sample'] = enc.decode(_pred_tokens).split('\n\n')[0].strip() 
        
        # Only include entries that have both references and sample
        references = [refer_dict[s]['references'] for s in refer_dict if 'sample' in refer_dict[s]]
        hypothesis = [refer_dict[s]['sample'] for s in refer_dict if 'sample' in refer_dict[s]]

        if args.ref_type == 'e2e':
            with open(args.output_ref_file, 'w', encoding='utf-8') as ref_writer:
            ...
            reference_writers = [
                open(os.path.join(args.output_ref_file, f'reference{fid}'), 'w', encoding='utf-8') 
