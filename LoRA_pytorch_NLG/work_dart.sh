...
# DART dataset PyTorch version experiment script - Optimized Configuration
# Based on https://github.com/coder-yd/Lora_jittor best practices
# Process: train -> generate -> decode -> evaluate

echo "=== DART Dataset PyTorch Version LoRA Experiment (Single) ==="
...
python src/gpt2_beam.py \
    ...
    --eval_len 64 \
    --model_card gpt2.sm \
    --init_checkpoint ./trained_models/GPT2_SM/dart/model.1000.pt \
    --platform single \
    --lora_dim 4 \
    --lora_alpha 32 \
    --beam 5 \
...
python src/gpt2_decode.py \
    ...
    --ref_type dart \
    --ref_num 6 \
    --output_ref_file ./eval/data/references_dart_pytorch \
    --output_pred_file ./eval/data/hypothesis_dart_pytorch \
    --tokenize --lower \
...
if [ -d "./eval/GenerationEval/" ]; then
    ...
    python eval.py \
        -R data/references_dart_pytorch/reference \
        -H data/hypothesis_dart_pytorch \
        -nr 6 \
        -m bleu,meteor,ter \
...
echo "=== DART PyTorch Version Experiment Complete ==="
echo "End time: $(date)"
