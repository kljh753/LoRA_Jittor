...
# E2E dataset PyTorch version experiment script - Optimized Configuration
# Based on https://github.com/coder-yd/Lora_jittor best practices
# Process: train -> generate -> decode -> evaluate

echo "=== E2E Dataset PyTorch Version LoRA Experiment (Single) ==="
...
python src/gpt2_beam.py \
    ...
    --eval_len 64 \
    --model_card gpt2.sm \
    --init_checkpoint ./trained_models/GPT2_SM/e2e/model.1000.pt \
    --platform single \
    --lora_dim 4 \
    --lora_alpha 32 \
    --beam 5 \
...
python src/gpt2_decode.py \
    --vocab ./vocab \
    --sample_file ./trained_models/GPT2_SM/e2e/predict.1000.b5p08.jsonl \
    --input_file ./data/e2e/test.jsonl \
    --ref_type e2e \
    --ref_num 8 \
    --output_ref_file ./eval/data/references_e2e_pytorch \
    --output_pred_file ./eval/data/hypothesis_e2e_pytorch \
    --tokenize --lower \
...
if [ -d "./eval/GenerationEval/" ]; then
    ...
    python eval.py \
        -R data/references_e2e_pytorch/reference \
        -H data/hypothesis_e2e_pytorch \
        -nr 8 \
        -m bleu,meteor,ter \
...
echo "=== E2E PyTorch Version Experiment Complete ==="
echo "End time: $(date)"
