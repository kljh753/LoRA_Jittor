...
# WebNLG dataset PyTorch version experiment script - Optimized Configuration
# Based on https://github.com/coder-yd/Lora_jittor best practices
# Process: train -> generate -> decode -> evaluate

echo "=== WebNLG Dataset PyTorch Version LoRA Experiment (Single) ==="
...
python src/gpt2_beam.py \
    ...
    --eval_len 64 \
    --model_card gpt2.sm \
    --init_checkpoint ./trained_models/GPT2_SM/webnlg/model.1000.pt \
    --platform single \
    --lora_dim 4 \
    --lora_alpha 32 \
    --beam 5 \
...
python src/gpt2_decode.py \
    ...
    --ref_type webnlg \
    --ref_num 4 \
    --output_ref_file ./eval/data/references_webnlg_pytorch \
    --output_pred_file ./eval/data/hypothesis_webnlg_pytorch \
    --tokenize --lower \
...
if [ -d "./eval/GenerationEval/" ]; then
    ...
    python eval.py \
        -R data/references_webnlg_pytorch/reference \
        -H data/hypothesis_webnlg_pytorch \
        -nr 4 \
        -m bleu,meteor,ter \
...
echo "=== WebNLG PyTorch Version Experiment Complete ==="
echo "End time: $(date)"
